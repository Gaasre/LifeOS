...
LifeOS note: the official ReUI skill has been installed locally, but ReUI MCP configuration is intentionally not retained in this repo. When MCP tools are unavailable, use the skill workflow with the shadcn CLI and the `@reui` registry fallback instead of configuring MCP unless the user explicitly asks for it.
